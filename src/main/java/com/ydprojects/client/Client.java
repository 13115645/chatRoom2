...
public class Client {
    ...
    private static void readFromServer(Socket sockett) throws IOException, ClassNotFoundException {
        ...
        while ((message = ((Message) objectInputStream.readObject())) != null) {
            messageText = MessageEncryption.decrypt(message.getMessage());

            if (message.getSeverCommand() != null) {
                switch (message.getSeverCommand()) {
                    case TERMINATE_CLIENT:
                        closeSocketAndExitSystem();
                        break;
                    case SERVER_SHUTDOWN_REQUEST:
                        startShutdownProgressBar();
                        break;
                    case SERVER_KICK_REQUEST:
                        JOptionPane.showMessageDialog(frmClient, "you have been removed from the server");
                        closeSocketAndExitSystem();
                        break;
                    case CLOSE_INSTANTLY:
                    ...
                    case ABORT_SHUTDOWN:
                        ...
                        break;
                    case STAY_CONNECTED:
                        MessageUtils.appendToPane(msg_area, messageText + "\n", Color.BLACK);
                    default:
                        LOG.info("default");
    ...
    private static void closeSocketAndExitSystem() throws IOException {
